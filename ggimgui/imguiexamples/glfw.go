...
type GLFW struct {
	...
	prevMouseButtonCallback func(*glfw.Window, glfw.MouseButton, glfw.Action, glfw.ModifierKey)
	prevScrollCallback      func(*glfw.Window, float64, float64)
	prevKeyCallback         func(*glfw.Window, glfw.Key, int, glfw.Action, glfw.ModifierKey)
	prevCharCallback        func(*glfw.Window, rune)
}
...
func (platform *GLFW) installCallbacks() {
	platform.prevMouseButtonCallback = platform.window.SetMouseButtonCallback(platform.mouseButtonChange)
	platform.prevScrollCallback = platform.window.SetScrollCallback(platform.mouseScrollChange)
	platform.prevKeyCallback = platform.window.SetKeyCallback(platform.keyChange)
	platform.prevCharCallback = platform.window.SetCharCallback(platform.charChange)
}
...
func (platform *GLFW) mouseButtonChange(window *glfw.Window, rawButton glfw.MouseButton, action glfw.Action, mods glfw.ModifierKey) {
	if platform.prevMouseButtonCallback != nil {
		platform.prevMouseButtonCallback(window, rawButton, action, mods)
	}
...
func (platform *GLFW) mouseScrollChange(window *glfw.Window, x, y float64) {
	if platform.prevScrollCallback != nil {
		platform.prevScrollCallback(window, x, y)
	}
...
func (platform *GLFW) keyChange(window *glfw.Window, key glfw.Key, scancode int, action glfw.Action, mods glfw.ModifierKey) {
	if platform.prevKeyCallback != nil {
		platform.prevKeyCallback(window, key, scancode, action, mods)
	}
...
func (platform *GLFW) charChange(window *glfw.Window, char rune) {
	if platform.prevCharCallback != nil {
		platform.prevCharCallback(window, char)
	}
